...
description: 通用设计工作流——五层顺序执行模型。按「第一层 场景 → 第二层 感知物料 → 第三层 形式组织 → 第四层 信息叙事 → 第五层 主观输出」逐层操作；各层为可编辑模块（场景层合并为单文件，其余层按子位置分文件）。支持三种调用模式：领域适配（默认）/ 吸睛 / AI自主。专项方法论（数据图表/图形字/图标批量/照片手绘线稿/前端设计/移动端 App/隐喻统摄叙事长页）已抽离为 subskills/ 子技能，按载体/任务特征路由。当需要按流程一步步构思/审视设计时触发。触发词：「设计工作流」「五层设计」「按流程做设计」「逐层设计」「帮我搭设计」。
version: 1.6.2
---
